...
CONTAINS
  ...
  SUBROUTINE READ_EVAC(IMODE)
    ...
    NAMELIST /PERS/ FYI,ID,DIAMETER_DIST,VELOCITY_DIST, &
         ...
         OUTPUT_MOTIVE_ANGLE, OUTPUT_ANGLE, OUTPUT_CONTACT_FORCE, OUTPUT_TOTAL_FORCE, OUTPUT_ACCELERATION, &
	 OUTPUT_GROUP_FORCE, OUTPUT_HAZARD_FORCE, &
...
237      REWIND(LU_INPUT) ; INPUT_FILE_LINE_NUMBER = 0
    ...
    SUBROUTINE READ_PERS
      ...
      !Smoke resistence to avoid agents going through hazards
...
24    REWIND(LU_INPUT)
      ...
      IF (OUTPUT_GROUP_FORCE) EVAC_N_QUANTITIES = EVAC_N_QUANTITIES + 1
      IF (OUTPUT_HAZARD_FORCE) EVAC_N_QUANTITIES = EVAC_N_QUANTITIES + 1
	 ...
	 IF (OUTPUT_HAZARD_FORCE) THEN
            EVAC_QUANTITIES_INDEX(n)=253
            n = n + 1
         END IF
...
!
  SUBROUTINE EVACUATE_HUMANS(TIN,DT,NM,ICYC)
    ...
    HUMAN_TIME_LOOP: DO WHILE ( DT_SUM < DT )
	  ...
	  !Temp Use to Show FX_Hazard
	  HR%FX_Group = 0.0_EB 		!HR%FX_Hazard	!0.0_EB
	  HR%FY_Group = 0.0_EB		!HR%FX_Hazard	!0.0_EB
...
!!$                     (1.0_EB+A_HAWK_T_FACTOR*MAX(0.0_EB,T-A_HAWK_T_START))*T_tmp >= Epsilon_Hawk) THEN   ! Was a dove
	  ...
	  HR%F_X = HR%F_X + HR%FX_Hazard
          HR%F_Y = HR%F_Y + HR%FY_Hazard
    ...
    HUMAN_CLASS_LOOP: DO N = 1, N_EVAC
       ...
       PLOOP: DO I = 1, N_HUMANS
	     ...
	     CASE(252)  !GROUP_FORCE, agent group force
                QP(NPP,NN) = REAL(SQRT(HR%FX_Group**2 + HR%FY_Group**2),FB)
	     CASE(253)  !HAZARD_FORCE, hazard force to agents
                QP(NPP,NN)= REAL(SQRT(HR%FX_Hazard**2 + HR%FY_Hazard**2),FB)
             END SELECT
          END DO
